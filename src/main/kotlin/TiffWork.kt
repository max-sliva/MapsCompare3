...
//            grForLabel2d.drawRect(dragStartX, dragStartY, e!!.x-dragStartX, e!!.y-dragStartY)
            ...
            grForLabel2d?.drawImage(cropImage(image as BufferedImage, square.x-3, square.y-3, square.width+3, square.height+3, imageScale), null, square.x+2, square.y+2)
            grForLabel2d?.draw(square)
...
//            imageScale = newWidth.toDouble() / image?.width!!
...
//        imageScale = (imgIcon.iconWidth.toDouble()+100) / image?.width!!
...
//        myFrame.repaint()
    ...
    cropBtn.addActionListener {
        val image = cropImage(image as BufferedImage, square.x, square.y, square.width, square.height, imageScale)
        imgIcon.image = image
        imgLabel?.icon = null
        imgLabel?.icon = imgIcon
    }

    //todo add button for scaling image to the size of window
...
fun cropImage(originalImgage: BufferedImage, x: Int, y: Int, w: Int, h: Int, imageScale: Double = 1.0): BufferedImage? {
    var croppedImage = originalImgage.getSubimage((x*imageScale).toInt(), (y*imageScale).toInt(), (w*imageScale).toInt(), (h*imageScale).toInt())
    croppedImage = croppedImage?.getScaledInstance((croppedImage.width/imageScale).toInt(), -1, Image.SCALE_FAST)?.toBufferedImage()
    return croppedImage
}
